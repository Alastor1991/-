...
import { UserProfile, ForumPost, Notification } from '../types';
...
const Profile: React.FC<ProfileProps> = ({ user, onUpdateUser, onLogout, onClose, readOnly = false }) => {
    ...
    const [activeTab, setActiveTab] = useState<'info' | 'saved' | 'inbox'>('info');
    ...
    useEffect(() => {
        const loadExtraData = async () => {
            if (!readOnly) {
                ...
                const saved = posts.filter(p => user.savedPostIds?.includes(p.id));
                setSavedPosts(saved);

                const notifs = await backend.getNotifications();
                setNotifications(notifs);
                if (activeTab === 'inbox') {
                    backend.markNotificationsRead();
                }
            }
        };
        loadExtraData();
    }, [user, activeTab, readOnly]);
    ...
    return (
        <div className="min-h-screen bg-[#050505] p-4 md:p-8 flex items-center justify-center relative overflow-hidden">
            ...
            {/* MAIN CARD */}
            <div className="relative w-full max-w-5xl z-10 animate-fade-in mb-20">
                ...
                {!readOnly && (
                    <div className="flex gap-2 mb-4 ml-4">
                        <button 
                            ...
                            className={`px-4 py-2 rounded-t-lg font-bold text-xs uppercase tracking-widest border-t border-l border-r ${activeTab === 'info' ? 'bg-[#121212] text-white border-white/10' : 'bg-black text-gray-600 border-transparent hover:text-white'}`}
                        >
                            Identity
                        </button>
                        <button 
                            onClick={() => setActiveTab('saved')} 
                            className={`px-4 py-2 rounded-t-lg font-bold text-xs uppercase tracking-widest border-t border-l border-r ${activeTab === 'saved' ? 'bg-[#121212] text-white border-white/10' : 'bg-black text-gray-600 border-transparent hover:text-white'}`}
                        >
                            Saved Signals ({savedPosts.length})
                        </button>
                        <button 
                            onClick={() => setActiveTab('inbox')} 
                            className={`relative px-4 py-2 rounded-t-lg font-bold text-xs uppercase tracking-widest border-t border-l border-r ${activeTab === 'inbox' ? 'bg-[#121212] text-white border-white/10' : 'bg-black text-gray-600 border-transparent hover:text-white'}`}
                        >
                            Inbox
                            {notifications.some(n => !n.read) && <span className="absolute top-1 right-1 w-2 h-2 bg-neon-red rounded-full animate-pulse"></span>}
                        </button>
                    </div>
                )}

                <div className="bg-[#121212]/90 backdrop-blur-xl border border-white/10 rounded-3xl rounded-tl-none overflow-hidden shadow-2xl relative group">
                    ...
                    {activeTab === 'saved' && (
                        <div className="p-8 min-h-[400px]">
                            <h2 className="text-2xl font-marker text-white mb-6">Archived Transmissions</h2>
                            {savedPosts.length > 0 ? (
                                <div className="grid grid-cols-1 gap-4">
                                    {savedPosts.map(post => (
                                        <div key={post.id} className="bg-black/40 border border-white/10 p-4 rounded flex justify-between items-center hover:border-neon-blue/50 transition-colors">
                                            <div>
                                                <div className="font-bold text-white mb-1">{post.title}</div>
                                                <div className="text-xs text-gray-500">by u/{post.author} in {post.communityId}</div>
                                            </div>
                                            <button onClick={() => onClose && onClose()} className="text-neon-blue text-xs uppercase border border-neon-blue px-3 py-1 rounded hover:bg-neon-blue hover:text-black transition-colors">View in Hub</button>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                                <div className="text-gray-600 font-mono text-center py-20">No signals saved in memory.</div>
                            )}
                        </div>
                    )}
                    ...
                    {activeTab === 'inbox' && (
                        <div className="p-8 min-h-[400px]">
                            <h2 className="text-2xl font-marker text-white mb-6">VoxTek Secure Inbox</h2>
                            {notifications.length > 0 ? (
                                <div className="space-y-2">
                                    {notifications.map(n => (
                                        <div key={n.id} className={`p-4 border-l-4 ${n.read ? 'border-gray-700 bg-black/20' : 'border-neon-red bg-neon-red/5'} mb-2`}>
                                            <div className="text-gray-400 text-[10px] uppercase mb-1 font-bold">{n.type} • {new Date(n.timestamp).toLocaleDateString()}</div>
                                            <div className="text-white">{n.message}</div>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                                <div className="text-gray-600 font-mono text-center py-20">Inbox empty. You are unpopular.</div>
                            )}
                        </div>
                    )}
                </div>
